...
namespace Nito.KitchenSink
{
    ...
    public sealed class SimplePropertyPath : NotifyPropertyChangedBase<SimplePropertyPath>, IDisposable
    {
        ...
        /// </summary>
        private static readonly TraceSource trace = new TraceSource("System.Windows.Data");
        ...
        public object Value
        {
            ...
            set
            {
                ...
                {
                    trace.TraceEvent(TraceEventType.Error, 0, "Property path not active; ignoring request to set Value.");
                    return;
                }

                if (!this.subscriptions[this.subscriptions.Length - 1].IsSubscribed)
                {
                    trace.TraceEvent(TraceEventType.Error, 0, "Property path not fully subscribed; ignoring request to set Value.");
                    return;
                }
        ...
        private sealed class SubscriptionStep
        {
            ...
            public object Evaluate()
            {
                ...
                if (info == null)
                {
                    trace.TraceEvent(TraceEventType.Error, 0, "Property '" + this.Name + "' not found for type '" + this.Object.GetType().Name + "', object '" + this.Object.ToString() + "'");
                    return null;
                }
                ...
                if (ret == null)
                {
                    trace.TraceEvent(TraceEventType.Error, 0, "Property '" + this.Name + "' is null for type '" + this.Object.GetType().Name + "', object '" + this.Object.ToString() + "'");
                }
